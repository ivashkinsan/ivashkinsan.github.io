...
P = (i) =>
    i < 0 ||
    A[i] || // is valid and note not playing?
    ((k = eval(`K` + i)), // get key
                    ...
                    .connect(
                        ...
                        (o.g = C.createGain(
                            ...
                            (o.frequency.value = // set frequency
                                ...
                                55 * // A 55 root note
                                2 **
                                ((i + 3) / 12 + // music scale formula
                                    O.value * 1 + // octave control
                                    Y.value / 12))
                        ))
                    ) // key control
                    .connect(C.destination), // gain to destination
                (o.g.gain.value = 0.2 / (1 + Math.log2(j))), // set gain
                o.start(), // play sound
                o
            ) // return sound
        )));
